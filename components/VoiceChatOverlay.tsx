...
export const VoiceChatOverlay: React.FC<VoiceChatOverlayProps> = ({ onClose, podcastContext }) => {
  ...
  useEffect(() => {
    ...
    const systemInstruction = t('voice_chat.system_instruction', { context: podcastContext });
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white shadow-2xl w-full max-w-2xl h-[80vh] flex flex-col relative">
        <button onClick={onClose} className="absolute top-3 right-3 text-gray-400 hover:text-gray-800 transition-colors z-10">
          <CloseIcon />
        </button>
        
        <div className="p-6 border-b">
            <h2 className="font-serif text-2xl font-bold">{t('voice_chat.title')}</h2>
            <p className="text-gray-600">{t('voice_chat.subtitle')}</p>
        </div>

        <div className="flex-1 overflow-y-auto p-6 space-y-4">
            {transcript.map((turn, index) => (
                <div key={index}>
                    <p className="text-right"><span className="bg-politiken-red text-white px-3 py-2 inline-block">{turn.user}</span></p>
                    <p className="mt-2"><span className="bg-gray-200 text-politiken-charcoal px-3 py-2 inline-block">{turn.model}</span></p>
                </div>
            ))}
             {transcript.length === 0 && status === 'listening' && (
                <div className="text-center text-gray-400 pt-16">
                    <p>{t('voice_chat.initial_prompt')}</p>
                </div>
            )}
        </div>

        <div className="p-6 border-t flex flex-col items-center justify-center">
            <div className={`h-20 w-20 flex items-center justify-center transition-colors ${status === 'listening' || status === 'speaking' ? 'bg-politiken-red' : 'bg-gray-300'}`}>
               ...
               {status === 'connecting' && <div className="h-5 w-5 border-2 border-t-transparent border-white animate-spin"></div>}
            </div>
             <p className="mt-4 text-sm text-gray-500 font-medium tracking-wide uppercase">
                {status === 'connecting' && t('voice_chat.status_connecting')}
                {status === 'listening' && t('voice_chat.status_listening')}
                {status === 'speaking' && t('voice_chat.status_speaking')}
                {status === 'error' && t('voice_chat.status_error')}
            </p>
        </div>
      </div>
    </div>
